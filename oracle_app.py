"""
Oracle — Institutional-Grade Options Intelligence Dashboard
============================================================
...
st.set_page_config(
    page_title="Oracle — Options Intelligence",
    page_icon="🔮",
    layout="wide",
...
.oracle-card { background: #0e1520; border: 1px solid #2a1a4a; border-radius: 8px; padding: 16px; margin: 6px 0; }
...
with st.sidebar:
    ...
    st.markdown("---")
    scan_btn = st.button("🔮 Run Oracle Scan")
...
<div style='padding:8px 0 16px 0;'>
  <h1 style='font-family:"Syne",sans-serif;font-size:2.2rem;font-weight:800;
             ...
             margin:0;letter-spacing:-0.02em;'>🔮 Oracle</h1>
...
def run_oracle_scan(tickers, max_exp, whale_thresh):
    gex_data   = {}
    whale_rows = []
    chain_data = {}
    ...
    with st.status("🔮 Running Oracle scan...", expanded=True) as status:
        pb  = st.progress(0)
        stx = st.empty()
        ...
        pb.empty(); stx.empty()
        status.update(label="✅ Oracle scan complete!", state="complete")
...
if scan_btn:
    run_oracle_scan(all_tickers, max_expiries, whale_threshold)
    ...
    if not st.session_state.gex_data:
        ...
        <div style='text-align:center;padding:60px 0;color:#2a1a4a;'>
          ...
          <div style='font-family:"JetBrains Mono",monospace;color:#5a3a80;margin-top:12px;'>
            Run Oracle Scan to see GEX/VEX maps
          </div>
        </div>""", unsafe_allow_html=True)
    else:
...
with tab2:
    ...
    if st.session_state.whale_data.empty:
        ...
        <div style='text-align:center;padding:60px 0;color:#2a1a4a;'>
          <div style='font-size:3rem;'>🐋</div>
          <div style='font-family:"JetBrains Mono",monospace;color:#5a3a80;margin-top:12px;'>
            Run Oracle Scan to detect whale activity
          </div>
        </div>""", unsafe_allow_html=True)
    else:
...
with tab3:
    if not st.session_state.chain_data:
        ...
        <div style='text-align:center;padding:60px 0;color:#2a1a4a;'>
          <div style='font-size:3rem;'>📋</div>
          <div style='font-family:"JetBrains Mono",monospace;color:#5a3a80;margin-top:12px;'>
            Run Oracle Scan to see contract intelligence
          </div>
        </div>""", unsafe_allow_html=True)
    else:
...
<div style='text-align:center;font-family:"JetBrains Mono",monospace;font-size:0.68rem;color:#1a0a30;'>
  Oracle · yfinance EOD data · GEX approximated from OI + Black-Scholes · Not financial advice
</div>""", unsafe_allow_html=True)
